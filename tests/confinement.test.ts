...
async function fixture() {
  ...
  const declaration = `${configPath}.declaration.json`;
  const declarationBinding = `${configPath}.declaration-binding.json`;
  ...
  for (const [file, mode] of [
    ...
    [manifest, 0o600],
    [declaration, 0o600],
    [declarationBinding, 0o600],
    [artifact, 0o700],
  ] as const) await fs.writeFile(file, "fixture\n", { mode });
  ...
  return { uid, config, configPath, artifact, manifest };
}
...
test("remote confinement rejects symlink and writable policy substitution", async () => {
  ...
  await assert.rejects(assertRemoteConfinement(value.config, value.configPath, value.artifact, "controlled", value.uid, value.uid), (error: unknown) => error instanceof OpsHavenError && error.code === "POLICY_DENIED");
