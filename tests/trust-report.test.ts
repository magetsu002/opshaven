...
test("trust report derives access only from the signed build declaration", () => {
  assert.deepEqual(summarizeDeclaredAccess(declaration, "controlled"), {
    shellAccess: "denied",
    sudoAccess: "exact-reviewed-commands",
    writeAccess: ["approval replay state"],
    dockerSocketAccess: "unavailable",
  });
  assert.deepEqual(summarizeDeclaredAccess(declaration, "read-only"), {
    shellAccess: "denied",
    sudoAccess: "unavailable",
    writeAccess: ["request replay state"],
    dockerSocketAccess: "unavailable",
  });
});

test("plain trust report states the enforced boundary and remaining assumptions", () => {
  const report: OperatorTrustReport = {
    ...
    boundaryVerification: {
      ...
      assertions: [{ name: "interactive shell denied", passed: true, detail: "forced-command policy denial" }],
    },
