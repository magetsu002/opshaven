...
test("native Streamable HTTP handles discovery, calls, notifications, malformed input, and shutdown", async () => {
  ...
  try {
    ...
    const unacceptable = await post(started.url, modern("tools/list"), { accept: "application/json;q=0, text/event-stream", "mcp-protocol-version": CURRENT_MCP_PROTOCOL, "mcp-method": "tools/list" });
