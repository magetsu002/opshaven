...
export class FixedCommandRunner implements CommandRunner {
  constructor(private readonly spawnProcess: RunnerSpawnLike = spawn as unknown as RunnerSpawnLike) {}
  ...
  async run(executable: string, args: readonly string[], options: RunOptions): Promise<RunResult> {
    ...
    return await new Promise<RunResult>((resolve, reject) => {
      ...
      child.stdout.on("data", (chunk: Uint8Array) => consume(chunk, stdoutChunks));
      child.stderr.on("data", (chunk: Uint8Array) => consume(chunk, stderrChunks));
      ...
      child.on("close", (code: number | null) => {
        ...
        if (!isText(stdout) || !isText(stderr)) return fail(new OpsHavenError("BINARY_OUTPUT", "Binary command output rejected."));
        settled = true;
