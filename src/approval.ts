...
export function decodeApprovalPayload(value: string): ApprovalPayload {
  ...
  if (body.version !== 1 || typeof body.nonce !== "string" || !/^[a-f0-9]{32}$/.test(body.nonce) || typeof body.digest !== "string" || !/^[a-f0-9]{64}$/.test(body.digest) || typeof body.expectedState !== "string" || !/^[a-f0-9]{64}$/.test(body.expectedState) || typeof body.policyVersion !== "string" || !/^[A-Za-z0-9._-]{1,64}$/.test(body.policyVersion) || typeof body.expiresAt !== "string") throw new OpsHavenError("APPROVAL_INVALID", "Approval token is malformed.");
...
async function readOwnerOnly(filePath: string, label: string): Promise<Uint8Array> {
  const stat = await fs.lstat(filePath);
  if (!stat.isFile() || stat.isSymbolicLink() || (stat.mode & 0o077) !== 0) throw new OpsHavenError("APPROVAL_INVALID", `${label} must be a regular owner-only file.`);
...
export class ApprovalService {
  ...
  private async secret(): Promise<Uint8Array> {
    const value = await readOwnerOnly(this.config.secretFile, "Approval secret");
  ...
  async create(operation: ResolvedOperation, ttlSeconds = this.config.defaultTtlSeconds): Promise<{ token: string; digest: string; expiresAt: string }> {
    ...
    await fs.mkdir(path.join(this.config.directory, "pending"), { recursive: true, mode: 0o700 });
    await fs.mkdir(path.join(this.config.directory, "used"), { recursive: true, mode: 0o700 });
    ...
    const privateKey = await readOwnerOnly(this.config.signingPrivateKeyFile, "Approval signing key");
    const remoteSignature = sign(null, Buffer.from(encoded, "utf8"), privateKey).toString("base64url");
    await fs.writeFile(path.join(this.config.directory, "pending", body.nonce), canonicalize(body), { flag: "wx", mode: 0o600 });
  ...
  async consume(token: string, operation: ResolvedOperation): Promise<{ digest: string; authorization: RemoteAuthorization }> {
    ...
    const pending = path.join(this.config.directory, "pending", body.nonce);
    const used = path.join(this.config.directory, "used", body.nonce);
