...
import { rollbackRemoteSynchronizationTransaction, type RemoteTransactionRollbackReceipt } from "./transaction.js";
...
export async function rollbackRecordedSynchronization(
  setup: RemoteSetupConfig,
  transactionId: string,
  previousGenerationIdentity: string | null,
  transport: RemoteAdminTransport = new PinnedSshAdminTransport(setup),
  ...
  if (previousGenerationIdentity !== null && !/^[a-f0-9]{64}$/.test(previousGenerationIdentity)) throw new OpsHavenError("CONFIG_INVALID", "Previous generation identity is invalid.");
  const receipt = await rollbackRemoteSynchronizationTransaction(setup, transactionId, transport);
  if (receipt.restoredGenerationIdentity !== previousGenerationIdentity) throw new OpsHavenError("POLICY_DENIED", "Restored generation does not match the recorded previous generation.");
  return receipt;
}
...
export async function rollbackRemoteSetup(setup: RemoteSetupConfig, approved: boolean, transport: RemoteAdminTransport = new PinnedSshAdminTransport(setup)): Promise<RemoteCleanupReceipt> {
  ...
  if (result.code !== 0) throw new OpsHavenError("SSH_FAILED", "Remote rollback failed safely. Inspect recovery state with opshaven doctor --debug.", true, { rollbackDebug: safeDiagnostic(result.stderr.trim() || "no diagnostic") });
...
export async function uninstallRemoteSetup(setup: RemoteSetupConfig, approved: boolean, transport: RemoteAdminTransport = new PinnedSshAdminTransport(setup)): Promise<RemoteCleanupReceipt> {
  ...
  if (result.code !== 0) throw new OpsHavenError("SSH_FAILED", "Remote uninstall failed safely. Inspect recovery state with opshaven doctor --debug.", true, { uninstallDebug: safeDiagnostic(result.stderr.trim() || "no diagnostic") });
