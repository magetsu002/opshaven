...
function actionFor(health: InstallationHealthReport): RemoteSetupRepairPlan["action"] {
  if (!health.repairRequired && health.repairClassification === "NO_REPAIR_NEEDED") return "none";
...
function changesFor(health: InstallationHealthReport): readonly string[] {
  if (!health.repairRequired && health.repairClassification === "NO_REPAIR_NEEDED") {
  ...
  return Object.freeze([
    ...
    "Verify the recovery-evidence manifest before changing active paths.",
