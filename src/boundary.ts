...
import { createAuthenticatedRequest, loadClientProtocolContext, verifyAuthenticatedResponse } from "./remote/authenticated-protocol.js";
...
export interface BoundaryAssertion { name: string; passed: boolean; detail: string }
export interface BoundaryReport { ok: boolean; checkedAt: string; assertions: BoundaryAssertion[] }

async function runSsh(host: HostResource, stdin: string, command?: string, knownHostsFile = host.knownHostsFile): Promise<{ code: number | null; stdout: string; stderr: string }> {
  const args = buildSshArgs({ ...host, knownHostsFile });
  if (command) args.push(command);
  return await new Promise((resolve) => {
    const child = spawn("/usr/bin/ssh", args, { shell: false, stdio: ["pipe", "pipe", "pipe"], env: { PATH: "/usr/bin:/bin", LANG: "C", LC_ALL: "C" } });
    const stdout: Uint8Array[] = [];
    const stderr: Uint8Array[] = [];
    child.stdout.on("data", (chunk: Uint8Array) => stdout.push(chunk));
    child.stderr.on("data", (chunk: Uint8Array) => stderr.push(chunk));
    child.on("close", (code: number | null) => resolve({ code, stdout: Buffer.concat(stdout).toString("utf8"), stderr: Buffer.concat(stderr).toString("utf8") }));
...
export async function verifyBoundary(config: OpsHavenConfig, configPath: string, mode: "controlled" | "read-only" = "controlled"): Promise<BoundaryReport> {
  ...
  const baseRequest: RemoteRequest = { version: 1, requestId: "boundary-valid", operation: "get_host_summary", resourceId: host.id, args: { resourceId: host.id }, limits: { ...config.limits } };
  ...
  catch { replayDenied = false; }
  assertions.push(assertion("request replay denied", replayDenied, "atomic nonce consumption"));
  ...
  try { const parsed = JSON.parse(mutationWire.stdout) as Record<string, unknown>; mutationDenied = parsed.ok === false && (parsed.error as Record<string, unknown> | undefined)?.code === "REMOTE_PROTOCOL_INVALID"; }
  catch { mutationDenied = false; }
  assertions.push(assertion("request mutation denied", mutationDenied, "Ed25519 request signature"));
  ...
  if (responseEnvelope && typeof responseEnvelope === "object") {
    try {
      ...
      verifyAuthenticatedResponse({ ...candidate, payload: `${String(candidate.payload).slice(0, -1)}A` }, valid.requestHash, baseRequest.requestId, trust.capability, trust.responsePublicKey);
    } catch (error) { responseMutationDenied = error instanceof OpsHavenError; }
  }
  assertions.push(assertion("response mutation denied", responseMutationDenied, "Ed25519 response signature and result hash"));
  ...
  assertions.push(assertion("host-key mismatch denied", mismatch.code !== 0 && /Host key|known hosts|verification/i.test(mismatch.stderr), "strict pinned host-key verification"));
  const audit = await new AuditLog(config.audit.path).verify();
  assertions.push(assertion("audit chain valid", audit.valid, `${audit.records} records verified`));
  return { ok: assertions.every((item) => item.passed), checkedAt: new Date().toISOString(), assertions };
}

export function formatBoundaryReport(report: BoundaryReport): string {
  const lines = report.assertions.map((item) => `${item.passed ? "PASS" : "FAIL"}  ${item.name}: ${item.detail}`);
