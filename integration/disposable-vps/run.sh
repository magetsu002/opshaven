...
const config = {
  ...
  approvals: { directory: path.join(root, "approvals"), secretFile: path.join(root, "approval.key"), signingPrivateKeyFile: path.join(root, "approval-private.pem"), verificationPublicKeyFile: path.join(root, "approval-public.pem"), remoteUsedDirectory: path.join(root, "local-remote-used"), defaultTtlSeconds: 300 },
  secretFingerprints: [],
  resources: [
    { id: "host.fixture", kind: "host", address: "127.0.0.1", port: 22222, user: "opshaven", knownHostsFile: path.join(root, "known_hosts"), identityFile: path.join(root, "id_ed25519"), connectTimeoutMs: 5000 },
...
node "$DIR/generate-capability.mjs" "$GEN/local.config.json" "$GEN/approval-private.pem" "$ROOT/dist/src/remote/dispatcher.js" "$GEN/declaration.json" "$GEN/capability.json" "$GEN/declaration-binding.json"
...
for _ in $(seq 1 60); do
  if [[ -s "$GEN/commits.json" ]] && docker compose -f "$DIR/compose.yml" exec -T vps systemctl is-active --quiet ssh.service opshaven-fixture.service; then break; fi
  sleep 1
done
[[ -s "$GEN/commits.json" ]]
docker compose -f "$DIR/compose.yml" exec -T vps systemctl is-active --quiet ssh.service opshaven-fixture.service
for _ in $(seq 1 40); do if ssh-keyscan -p 22222 127.0.0.1 > "$GEN/known_hosts" 2>/dev/null; then break; fi; sleep 1; done
[[ -s "$GEN/known_hosts" ]]
...
[[ "$SHELL_RESULT" == *'POLICY_DENIED'* && "$SHELL_RESULT" != *'uid='* ]]
cp "$GEN/id_ed25519" "$GEN/stolen_id_ed25519"
chmod 600 "$GEN/stolen_id_ed25519"
STOLEN_SSH=(ssh -p 22222 -i "$GEN/stolen_id_ed25519" -o BatchMode=yes -o IdentitiesOnly=yes -o StrictHostKeyChecking=yes -o UserKnownHostsFile="$GEN/known_hosts" -o ClearAllForwardings=yes -o ForwardAgent=no -o RequestTTY=no opshaven@127.0.0.1)
STOLEN_SHELL="$("${STOLEN_SSH[@]}" /bin/sh 2>/dev/null || true)"
[[ "$STOLEN_SHELL" == *'POLICY_DENIED'* && "$STOLEN_SHELL" != *'uid='* ]]
STOLEN_UNSIGNED='{"version":1,"requestId":"stolen-key","operation":"get_host_summary","resourceId":"host.fixture","args":{"resourceId":"host.fixture"},"limits":{"timeoutMs":15000,"maxBytes":131072,"maxLines":1000}}'
STOLEN_REJECTED="$(printf '%s\n' "$STOLEN_UNSIGNED" | "${STOLEN_SSH[@]}")"
node -e 'const x=JSON.parse(process.argv[1]); if(x.ok || x.error?.code!=="REMOTE_PROTOCOL_INVALID") process.exit(1)' "$STOLEN_REJECTED"
...
printf 'disposable-vps: stolen restricted key contained, shell denial, pinned host keys, signed capabilities and declarations, authenticated requests and responses, confinement, boundary verification, exact deployment, failed-health restoration, rollback, approval rejection, and audit verification passed\n'
